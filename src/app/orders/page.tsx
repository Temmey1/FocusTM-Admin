...
const statusStyle: Record<OrderStatus,string> = {
  pending:    "border-ftm-linelt text-ftm-muted",
  paid:       "border-ftm-white text-ftm-white",
  processing: "border-ftm-offwhite text-ftm-offwhite",
  shipped:    "border-ftm-offwhite text-ftm-offwhite",
  completed:  "border-ftm-white text-ftm-white",
...
export default function OrdersPage() {
  ...
  return (
    <div>
      ...
      {loading ? <p className="text-ftm-muted text-[12px]">Loading...</p> : orders.length === 0 ? (
            ...
            <tbody>
              {orders.map((o,i) => (
                <tr key={o.id} className={`border-t border-ftm-line ${i%2===0?"bg-ftm-black":"bg-ftm-deep"}`}>
                  ...
                  <td className={tdCls}>
                    <button onClick={()=>setDetail(o)} className="text-ftm-muted hover:text-ftm-white transition-colors text-[10px] uppercase tracking-wider">View</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {detail && (
        <>
          ...
          <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-ftm-deep border-l border-ftm-line overflow-y-auto p-8">
            ...
            <p className="font-mono text-[12px] text-ftm-offwhite mb-5">{detail.orderNumber || detail.id}</p>
